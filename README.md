...
本仓库是 RustDesk 的定制分支（产品分支 `rustdesk-tiny`，上游基线 `978e2e28b`），**只支持 IP 地址直连** 的远程桌面能力。与原版的主要差异：

- **仅支持 IP:端口直连**：只接受显式的数字 IPv4/IPv6 地址加非零端口；**屏蔽了一切与 RustDesk 官方服务器或自建服务器的通信**——不注册会合/中继服务器、不使用 RustDesk ID、无 NAT 探测、延迟探测、账号同步和软件自动更新，空闲时不会向任何 RustDesk 服务器发送信息。两台设备之间的连接数据也不经过任何第三方服务器。
